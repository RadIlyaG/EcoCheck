  ...
  pack [TitleFrame $mf.frChooseMode -text "Choose Mode" -bd 2 -relief groove] -padx 2 -pady 2 -fill both
    ...
    set gaApprGui(frANewListBox) [frame $fr.frANewListBox -bd 0 -relief groove] 
      set fr123 [frame $gaApprGui(frANewListBox).fr123 -bd 2 -relief groove] 
        ...
        set gaApprGui(lbANew) [ttk::treeview $fr123.lbANew -yscrollcommand "$fr123.yscroll set" \
...
proc CheckRnADB {} {
  ...
  puts "ecos:<$ecos>"
  dataBase close
  # $gaApprGui(lbANew) delete [$gaApprGui(lbANew) children root]
  if [llength $ecos] {
    foreach eco $ecos {
      $gaApprGui(lbANew) insert {} end -id $eco -text $eco
...
proc EcoToHandle {} {
  global gaApprGui
  set cell [$gaApprGui(lbANew) selection]
  $gaApprGui(entEco) delete 0 end
...
proc CheckAIDB {} {
  ...
  #puts "units:<$units>"
  dataBase close
  #$gaApprGui(lbAI) delete [$gaApprGui(lbAI) children]
  if [llength $units] {
    foreach unit $units {
